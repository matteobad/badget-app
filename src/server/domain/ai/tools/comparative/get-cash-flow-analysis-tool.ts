...
import { cashFlowArtifact } from "../../artifacts/cash-flow";
...
export const cashFlowTool = z.object({
  from: z
    .string()
...
export const getCashFlowAnalysisTool = tool({
  description:
    "Generate a detailed personal cash flow analysis with interactive charts, spending trends, and practical insights. Use this tool when users want to understand their income versus expenses, visualize their financial habits, or get actionable advice to improve their personal financial health.",
  inputSchema: cashFlowTool.omit({ showCanvas: true }), // Remove showCanvas since this always shows canvas
    ...
    try {
      ...
      const analysis = cashFlowArtifact.stream({
        ...
        toast: {
          ...
          currentStep: 0,
          totalSteps: 4,
          currentLabel: "Loading cash flow data",
      ...
      const initialMessageStream = streamText({
        ...
        temperature: 0.2,
        system: `
          You are a financial assistant generating a brief initial message for a cash flow analysis. 
