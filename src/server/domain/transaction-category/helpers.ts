...
export async function generateCategoryEmbedding(
  ...
  try {
    ...
    const existingEmbedding = await db
      .select({ name: transaction_category_embeddings_table.name })
      .from(transaction_category_embeddings_table)
      .where(eq(transaction_category_embeddings_table.name, name))
      .limit(1);

    if (existingEmbedding.length > 0) {
      console.info(`Embedding already exists for category: "${name}"`);
    ...
    console.info(`Generated embedding for category: "${name}"`);
  ...
  try {
    ...
    const existingEmbeddings =
      categoryNames.length > 0
        ? await db
            ...
            .where(
              inArray(
                transaction_category_embeddings_table.name,
                categoryNames,
              ),
            )
        : [];
    ...
    for (const cat of categories) {
      if (existingNames.has(cat.name)) {
        console.info(`Embedding already exists for category: "${cat.name}"`);
    ...
    const promises = categoriesToProcess.map(async (category, index) => {
      try {
        ...
        console.info(`Generated embedding for category: "${category.name}"`);
        return {
          name: category.name,
          success: true,
        };
